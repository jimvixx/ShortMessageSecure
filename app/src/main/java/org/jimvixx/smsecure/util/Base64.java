...
public class Base64 {
  ...
  public static String encodeBytesWithoutPadding(byte[] source, int offset, int length) {
    ...
    if (len >= 2 && encoded.charAt(len - 2) == '=') {
      return encoded.substring(0, len - 2);
    } else if (len >= 1 && encoded.charAt(len - 1) == '=') {
      return encoded.substring(0, len - 1);
    } else {
      return encoded;
    }
  }
  ...
  private static int decode4to3(
          byte[] source, @SuppressWarnings("SameParameterValue") int srcOffset,
          byte[] destination, int destOffset, int options) {
    ...
    // Example: Dk==
    if (s2 == EQUALS_SIGN) {
      int outBuff = ((DECODABET[s0] & 0xFF) << 18)
              | ((DECODABET[s1] & 0xFF) << 12);
    ...
    if (bytes.length >= 4 && !dontGunzip) {
      ...
      if (java.util.zip.GZIPInputStream.GZIP_MAGIC == head) {
        ...
        try {
          ...
          // if GZIP decoding fails, fall back to the originally decoded bytes
        }   // end catch
        finally {
          try {
            if (baos != null) baos.close();
  ...
  public static void decodeFileToFile(String infile, String outfile)
    ...
    try (java.io.OutputStream out =
      ...
      Log.w("Base64", "Failed to decode file to file", e);
      throw e;
    }
  }
    ...
    public void write(int theByte)
      ...
      // Else, Decoding
        ...
        if (decodabet[theByte & 0x7f] > WHITE_SPACE_ENC) {
          buffer[position++] = (byte) (theByte & 0x7f);
