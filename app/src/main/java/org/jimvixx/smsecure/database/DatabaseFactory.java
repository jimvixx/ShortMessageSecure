...
public class DatabaseFactory {
  ...
  public static MessageDatabase getMessageDatabase(Context context) {
    return getInstance(context).messageDatabase;
  }
  ...
  public void reset(Context context) {
    ...
    this.messageDatabase.reset(databaseHelper);
                                        ...
                                        DatabaseUpgradeActivity.DatabaseUpgradeListener listener)
  {
    ...
    listener.setProgress(1, total);
    db.beginTransaction();

    listener.setProgress(2, total);
    db.setTransactionSuccessful();

    listener.setProgress(3, total);
    db.endTransaction();

    listener.setProgress(4, total);
  ...
  private static class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
      ...
      if (oldVersion < INTRODUCED_IDENTITIES_VERSION) {
        db.execSQL("CREATE TABLE identities (_id INTEGER PRIMARY KEY, identity_key TEXT UNIQUE, name TEXT UNIQUE, mac TEXT);");
      }
      ...
      if (oldVersion < INTRODUCED_TOFU_IDENTITY_VERSION) {
        ...
        db.execSQL("CREATE TABLE identities (_id INTEGER PRIMARY KEY, recipient INTEGER UNIQUE, identity_key TEXT, mac TEXT);");
      }
      ...
      if (oldVersion < MIGRATED_CONVERSATION_LIST_STATUS_VERSION) {
        ...
        try (Cursor threadCursor = db.query("thread", new String[]{"_id"}, null, null, null, null, null)) {
        ...
        for (long thread : threads) {
          ...
          try (Cursor cursor = db.rawQuery("SELECT DISTINCT date AS date_received, status " +
...
//        dropIndexIfExists(db, "mms_thread_id_index");
//        dropIndexIfExists(db, "mms_read_index");
//        dropIndexIfExists(db, "mms_read_and_notified_and_thread_id_index");
//        dropIndexIfExists(db, "mms_message_box_index");
//        dropIndexIfExists(db, "mms_date_sent_index");
//        dropIndexIfExists(db, "mms_thread_date_index");
//        dropIndexIfExists(db, "mms_addresses_mms_id_index");
//
