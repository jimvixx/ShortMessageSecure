...
/**
 * Database to supply all types of contacts that SMSecure needs to know about
 *
 * @author Jake McGinty
 */
public class ContactsDatabase {
  ...
  public @NonNull Cursor querySystemContacts(String filter) {
    ...
    return new ProjectionMappingCursor(cursor, projectionMap,
            new Pair<>(CONTACT_TYPE_COLUMN, NORMAL_TYPE));
  }

  public @NonNull Cursor querySMSecureContacts(String filter) {
    ...
    if (TextUtils.isEmpty(filter)) {
      cursor = context.getContentResolver().query(ContactsContract.Data.CONTENT_URI,
              ...
              new String[]{"vnd.android.cursor.item/vnd.org.jimvixx.smsecure.contact"},
              sort);
    } else {
      cursor = context.getContentResolver().query(ContactsContract.Data.CONTENT_URI,
              projection,
    ...
    return new ProjectionMappingCursor(cursor, projectionMap,
            new Pair<>(LABEL_COLUMN, "SMSecure"),
